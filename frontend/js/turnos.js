...
const reserva = { sucursal: '', servicio: '', tipo: '', profesional: '', fecha: '', hora: '' };
let pasoActual = 1;
...
function mostrarPaso(paso) {
  ...
  mensaje.textContent = '';
  actualizarResumen();
  window.scrollTo({ top: document.querySelector('.turnos-main').offsetTop - 20, behavior: 'smooth' });
}
...
function cargarTipos() {
  const opciones = {
    Veterinaria: ['Consulta general', 'Control de salud', 'Consulta dermatológica'],
  ...
  document.querySelector('#tipo-servicio').innerHTML = opciones[reserva.servicio].map((tipo) =>
...
function renderCalendario() {
  ...
  const hoy = new Date();
  let dias = ['L', 'M', 'X', 'J', 'V', 'S', 'D'].map((dia) => `<strong>${dia}</strong>`).join('');
  for (let i = 0; i < inicio; i += 1) dias += '<span></span>';
  for (let dia = 1; dia <= ultimoDia; dia += 1) {
    const fecha = new Date(year, month, dia);
    const disponible = fecha >= new Date(hoy.getFullYear(), hoy.getMonth(), hoy.getDate()) && fecha.getDay() !== 0;
  ...
  calendario.querySelectorAll('[data-month]').forEach((boton) => {
    boton.addEventListener('click', () => {
      mesCalendario.setMonth(mesCalendario.getMonth() + (boton.dataset.month === 'next' ? 1 : -1));
      renderCalendario();
    });
  });
}
...
function cargarHorarios(fecha) {
  ...
  contenedorHorarios.innerHTML = horarios.map((hora) =>
  ...
  contenedorHorarios.querySelectorAll('.horario').forEach((boton) => {
    boton.addEventListener('click', () => {
      ...
      contenedorHorarios.querySelectorAll('.horario').forEach((opcion) => opcion.classList.toggle('seleccionado', opcion === boton));
      actualizarNavegacion();
      actualizarResumen();
    });
  });
...
function actualizarResumen() {
  ...
  const datos = [['Sucursal', reserva.sucursal], ['Servicio', reserva.servicio], ['Atención', reserva.tipo], ['Profesional', reserva.profesional], ['Fecha', reserva.fecha ? formatoFecha(new Date(`${reserva.fecha}T12:00:00`)) : 'Pendiente'], ['Hora', reserva.hora || 'Pendiente']];
  resumen.innerHTML = datos.map(([titulo, valor]) => `<div><dt>${titulo}</dt><dd>${valor || 'Pendiente'}</dd></div>`).join('');
}

document.addEventListener('click', (evento) => {
  const accion = evento.target.closest('[data-action]');
  if (accion) {
    const acciones = { branch: 'sucursal', service: 'servicio', type: 'tipo', vet: 'profesional' };
  ...
  if (cambioMes) {
    mesCalendario.setMonth(mesCalendario.getMonth() + (cambioMes.dataset.month === 'next' ? 1 : -1));
    renderCalendario();
  }
});
...
anterior.addEventListener('click', () => mostrarPaso(Math.max(pasoActual - 1, 1)));

document.querySelector('#datos-form').addEventListener('submit', (evento) => {
  evento.preventDefault();
  const datos = new FormData(evento.currentTarget);
  const nombre = datos.get('nombre');
  document.querySelector('.turno-seccion[data-step="6"]').innerHTML = `<div class="turno-confirmado"><p class="eyebrow">Reserva confirmada</p><h2>¡Te esperamos, ${nombre}!</h2><p>Tu turno para ${reserva.sucursal} quedó reservado para el ${formatoFecha(new Date(`${reserva.fecha}T12:00:00`))} a las ${reserva.hora}.</p></div>`;
